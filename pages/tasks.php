...
<title>Tasks â€¢ Mobile Todo</title>
...
<?php endforeach; ?>
  ...
  <div class="task-main">
    <div class="item-info">
      ...
      <div class="item-sub">
        <?= $row['category_name'] ? 'Category: ' . htmlspecialchars($row['category_name']) . ' â€¢ ' : '' ?>
        Prioritas: <?= htmlspecialchars(ucfirst($row['priority'])) ?>
        <?= $row['due_date'] ? ' â€¢ Due: ' . htmlspecialchars($row['due_date']) : '' ?>
      </div>
    </div>
  ...
  <?php if ($cl['user_id'] == $user_id): // hanya user sendiri bisa toggle ?>
    <form method="post" class="checklist-form">
      ...
      <button class="check-btn <?= $cl['completed'] ? 'done' : '' ?>">
        <span class="check-icon"><?= $cl['completed'] ? 'âœ“' : 'â—‹' ?></span>
...
.check-name {
  ...
  .completed-task {
    background-color: #e0e0e0;  /* background lebih pucat */
    color: #666;               /* teks lebih pudar */
    text-decoration: line-through; /* coret teks */
}

.completed-task .checklist-users button {
    opacity: 0.6;  /* tombol checklist sedikit transparan */
}
